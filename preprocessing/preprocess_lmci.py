"""
Skull strip and affine registration script for MRI and PET for LMCI
"""
...
def exec_bet(input_path, output_path, frac="0.5"):
    command = ["bet", input_path, output_path, "-R", "-f", frac, "-g", "0"]
...
for sub_dir in SUBCATEGORY_DIR:
    ...
    for subject in subjects:
        ...
        affine_register(os.path.join(path_to_mri, 'mri_brain.nii.gz'), '/usr/local/fsl/data/standard/MNI152_T1_1mm_brain.nii.gz' , os.path.join(path_to_mri, 'mri_registered_brain.nii.gz'), os.path.join(path_to_mri, 'registered_mri.mat'))
        ...
        affine_register(os.path.join(path_to_pet, 'pet_brain.nii.gz'), os.path.join(path_to_mri, 'mri_registered_brain.nii.gz') , os.path.join(path_to_pet, 'pet_registered_brain.nii.gz'), os.path.join(path_to_pet, 'registered_pet.mat'))
